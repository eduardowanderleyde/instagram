class User < ApplicationRecord
  ...
  def follow(user)
    unless self.followings.include?(user) || self.waiting_followings.include?(user)
      Follow.create(follower: self, followed: user)
    end
  end

  def unfollow(user)
    follow_record = self.accepted_sent_requests.find_by(followed: user) || self.waiting_sent_requests.find_by(followed: user)
